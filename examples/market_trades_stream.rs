...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let timestamp = SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let log_filename = format!("market_trades_log_{}.txt", timestamp);
    ...
    // Initialize log file
    {
        ...
        writeln!(
            file,
            "Market Trades Monitor Log - Started at: {}",
            chrono::Local::now()
        )?;
    ...
    println!("Starting Orderly Network Market Trades Monitor");
    ...
    let wss_id = env::var("ORDERLY_WSS_ID").unwrap_or_else(|_| "market_trades_stream".to_string());
    ...
    println!("Subscribing to market trades stream...");
    log_to_file(
        &log_filename,
                ...
                match msg {
                    Ok(msg) => {
                        if let Message::Text(text) = &msg {
                            ...
                            // Process market trade data if received
                            if text.contains("\"topic\":\"market_trades\"") && text.contains("\"data\":") {
                                println!("📈 MARKET TRADE EVENT DETECTED 📈");
                                log_to_file(&log_filename, "MARKET TRADE EVENT DETECTED").await?;
                            }
                        } else {
