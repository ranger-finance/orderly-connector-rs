...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Orderly Network Mark Prices Monitor");
    ...
    let message_handler = Arc::new(|msg: String| {
        ...
        if let Ok(parsed) = serde_json::from_str::<Value>(&msg) {
            // Check if this is a mark prices message
    ...
    match client.subscribe_mark_prices().await {
        Ok(_) => println!("Successfully subscribed to mark prices. Waiting for updates..."),
        Err(e) => {
            eprintln!("Failed to subscribe to mark prices: {}", e);
            return Err(e.into());
        }
    }
