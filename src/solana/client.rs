...
///
/// This function does **not** sign the transaction. The returned transaction must be signed externally (e.g., by a wallet).
///
/// # Arguments
/// - `rpc_client`: Solana RPC client for fetching blockhash.
/// - `config`: Orderly Solana config.
/// - `user_pubkey`: The user's public key (as `Pubkey`). No private key required.
/// - `amount`: Amount of USDC to deposit (in smallest units).
/// - `orderly_account_id_hex`: 32-byte hex string for the Orderly account ID.
///
/// # Returns
/// An unsigned `VersionedTransaction` ready for external signing.
///
/// # Example
/// ```ignore
/// let tx = prepare_solana_deposit_tx_unsigned(
///     &rpc_client, &config, &user_pubkey, 1_000_000, "abcdef...")?;
/// // Send tx to wallet for signing
/// ```
pub fn prepare_solana_deposit_tx_unsigned(
    rpc_client: &RpcClient,
    config: &SolanaConfig,
    ...
    .map_err(|e| OrderlyError::NetworkError(e.to_string()))?;
    // Return unsigned transaction (no signatures)
